...
public class CircularOrbitAPIs<L, E> {
	...
	/**
	 * calcular the entropy of a circular orbit
	 * 
	...
	public double getObjectDistributionEntropy(CircularOrbit<L, E> circularOrbit) {
		...
		while(circularOrbit.getTracks().size() != trackMap.size()) {
			trackMap.put(new Track(0D), new HashSet<>());
		}
		int total = 0;
		...
		double entropy = 0;
		for(Entry<Track, Set<E>> entry : trackMap.entrySet()) {
			entropy += (entry.getValue().size() / (double)total) * (Math.log(entry.getValue().size() / (double)total));
		}
		return -entropy;
	}
	
	/**
	 * calcular the logical distance of two physical object in a circular orbit
	 * 
	...
	public int getLogicalDistance (CircularOrbit<? extends CentralObject, ? extends PhysicalObject> c, E e1, E e2) {
		...
		int dis = 1;
		int num = 0, first = 0, last = 0, right = 0;
		...
		right = num;
		dis ++;
		while(num < length && !queue.isEmpty()) {
			temp = queue.get(0);
			queue.remove(0);
			first ++;
	...
	/**
	 * calcular the physical distance of two physical object in a circular orbit
	 * 
	 * @param c the target circular orbit
	 * @param e1 one physical object
	 * @param e2 the other physical object
	...
	public Double getPhysicalDistance(CircularOrbit<L, E> c, E e1, E e2) {
		if(c instanceof ConcreteCircularOrbitWithoutPosition) {
	 ...
	 */
	public Difference getDifference (CircularOrbit<? extends CentralObject, ? extends PhysicalObject> c1, CircularOrbit<? extends CentralObject, ? extends PhysicalObject> c2) {
		...
		if(c1 instanceof AtomStructure && c2 instanceof AtomStructure) {
			...
			for(int i = 1; i <= ((trackNumber < 0) ? c2.getTracks().size() : c1.getTracks().size()); i ++) {
				...
				if(track1 == null) {
					...
					physicalObjectNumberDifference.put((double)i, c1.getPhysicalObjects(track1).size() - c2.getPhysicalObjects(track2).size());
				}
			}
			return difference;
		} else if((c1 instanceof PersonalAppEcosystem && c2 instanceof PersonalAppEcosystem)||(c1 instanceof StellarSystem && c2 instanceof StellarSystem)) {
